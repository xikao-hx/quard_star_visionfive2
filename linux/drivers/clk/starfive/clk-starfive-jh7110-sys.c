...
/* UART2 and QSPI are owned by hart4 FreeRTOS in the AMP configuration.
 * Linux has no consumers for them in its AMP DT, so keep their gates critical
 * to prevent the late unused-clock pass from disabling RTOS peripherals.
 */
#define JH7110_UART2_CLK_FLAGS			CLK_IS_CRITICAL
#define JH7110_QSPI_CLK_FLAGS			CLK_IS_CRITICAL
#else
#define JH7110_UART2_CLK_FLAGS			0
#define JH7110_QSPI_CLK_FLAGS			0
#endif
...
static const struct jh71x0_clk_data jh7110_sysclk_data[] __initconst = {
	...
	/* qspi */
	JH71X0_GATE(JH7110_SYSCLK_QSPI_AHB, "qspi_ahb", JH7110_QSPI_CLK_FLAGS,
		     JH7110_SYSCLK_AHB1),
	JH71X0_GATE(JH7110_SYSCLK_QSPI_APB, "qspi_apb", JH7110_QSPI_CLK_FLAGS,
		     JH7110_SYSCLK_APB_BUS),
	JH71X0__DIV(JH7110_SYSCLK_QSPI_REF_SRC, "qspi_ref_src", 16, JH7110_SYSCLK_PLL0_OUT),
	JH71X0_GMUX(JH7110_SYSCLK_QSPI_REF, "qspi_ref", JH7110_QSPI_CLK_FLAGS, 2,
		    JH7110_SYSCLK_OSC,
	...
	JH71X0_GATE(JH7110_SYSCLK_UART2_APB, "uart2_apb", JH7110_UART2_CLK_FLAGS,
		     JH7110_SYSCLK_APB0),
	JH71X0_GATE(JH7110_SYSCLK_UART2_CORE, "uart2_core", JH7110_UART2_CLK_FLAGS,
		     JH7110_SYSCLK_OSC),
