...
| `work/target/amp/gpt.img` | 写入 `0x080000` 的重定位 GPT |
| `work/target/amp/recovery.img` | 写入 `0x0C0000` 的初始 recovery |
...
| `work/target/amp/gpt-layout.txt` | 重定位 GPT 的 GUID、LBA、offset 和 size 检查报告 |
| `work/target/amp/nor-layout.txt` | NOR offset、size、更新策略和实际镜像大小检查报告 |
